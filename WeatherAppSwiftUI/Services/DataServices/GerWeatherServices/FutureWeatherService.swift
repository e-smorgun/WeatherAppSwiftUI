...
class FutureWeatherService {
    ...
    private let cachePath: URL = {
        ...
        return documentsDirectory.appendingPathComponent("dailyWeatherCache")
    }()
    
    func fetchWeatherByCoordinates(lat: String, lon: String, completion: @escaping (Result<WeatherResponse, Error>) -> Void) {
        if Reachability.isConnectedToNetwork() {
                   let url = URL(string: "https://api.openweathermap.org/data/2.5/forecast?lat=\(lat)&lon=\(lon)&appid=\(self.APIKey)")!
                   print(url)
    ...
    func fetchCurrentLocationWeather(completion: @escaping (Result<WeatherResponse, Error>) -> Void) {
        if Reachability.isConnectedToNetwork() {
               if let currentLocation = locationManager.currentLocation {
                   let url = URL(string: "https://api.openweathermap.org/data/2.5/forecast?lat=\(currentLocation.coordinate.latitude)&lon=\(currentLocation.coordinate.longitude)&appid=\(self.APIKey)")!
                   print(url)
                   ...
                   completion(.failure("Местоположение еще не определено" as! Error))
               }
        } else {
            print("data from cach: ")
            if let cachedData = cacheManager.loadFromCache(cachePath: cachePath) {
